...
namespace sambackend.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateUserProfile([FromBody] UserProfileEdit userProfileEdit)
        {
            ...
            try
            {
                var result = await _userService.UpdateUserAsync(Guid.Parse(userId), userProfileEdit);  // <-- Updated method name
        ...
        public async Task<IActionResult> DeleteUser(Guid userId)
        {
            ...
            {
                return StatusCode(500, new { status = "error", message = "An error occurred during deletion.", details = ex.Message });
            }
        }

        [HttpPut("update/{userId}")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> UpdateUser(Guid userId, [FromBody] UserProfileEdit userProfileEdit)
        {
            try
            {
                var result = await _userService.UpdateUserAsync(userId, userProfileEdit);  // <-- Updated method name
                if (result.Status == "Failure")
                {
            ...
            {
                return StatusCode(500, new { status = "error", message = "An error occurred during user update.", details = ex.Message });
            }
        }
    }
}
